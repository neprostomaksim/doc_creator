 ...
 */
const EXTRACT_RULES = `Верни СТРОГО валидный JSON-объект без пояснений и markdown-обёртки, вида:
...
- Бери только реально присутствующие данные, ничего не выдумывай.
- Если поля нет — просто не включай его.`;

export function buildExtractRequisitesPrompt(text: string): string {
  return `Извлеки из документа реквизиты ОДНОЙ организации (или ИП/физлица). Если сторон несколько — бери первую (Исполнитель / Продавец / первую упомянутую).
...
/** То же извлечение, но источник — изображение (скрин/фото документа). */
export function buildExtractRequisitesFromImagePrompt(): string {
  return `На изображении — документ (скрин/фото договора, карточки предприятия или реквизитов). Распознай текст и извлеки реквизиты ОДНОЙ организации (или ИП/физлица). Если сторон несколько — бери первую (Исполнитель / Продавец / первую упомянутую).
